...
def categorize_dish(dish_name: str, dish_ingredients: set[str]) -> str:
    ...
    match True:
        case _ if dish_ingredients <= VEGAN:
            return f"{dish_name}: VEGAN"
        case _ if dish_ingredients <= VEGETARIAN:
            return f"{dish_name}: VEGETARIAN"
        case _ if dish_ingredients <= PALEO:
            return f"{dish_name}: PALEO"
        case _ if dish_ingredients <= KETO:
            return f"{dish_name}: KETO"
        case _ if dish_ingredients <= OMNIVORE:
