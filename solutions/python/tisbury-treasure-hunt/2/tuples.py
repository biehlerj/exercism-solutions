...
def compare_records(azara_record: TreasureRecord, rui_record: RuiRecord) -> bool:
...
def create_record(
    azara_record: TreasureRecord, rui_record: RuiRecord
) -> CombinedRecord | str:
...
def clean_up(combined_record_group: tuple[CombinedRecord, ...]) -> str:
    ...
    combined_records = tuple(
        tuple([record[0]]) + record[2:] for record in combined_record_group
    )
